...
class DailyJobToSyncMetadataTest {
    ...
    fun `should sync active tv shows when job runs`() {
        ...
        whenever(tmdbClient.tvShowDetails(100)).thenReturn(updatedShow.right())
    ...
    fun `should sync active movies when job runs`() {
        ...
        whenever(tmdbClient.movieDetails(200)).thenReturn(updatedMovie.right())
    ...
    fun `should not sync released or canceled tv shows`() {
        ...
        whenever(tmdbClient.tvShowDetails(102)).thenReturn(updatedActiveShow.right())

        job.sync()

        verify(tmdbClient).tvShowDetails(102)
    }
    ...
    fun `should not sync released or canceled movies`() {
        ...
        whenever(tmdbClient.movieDetails(202)).thenReturn(updatedActiveMovie.right())

        job.sync()

        verify(tmdbClient).movieDetails(202)
    }
    ...
    fun `should log error and continue when tmdb fails for a tv show`() {
        ...
        whenever(tmdbClient.tvShowDetails(100)).thenReturn(TmdbError.MovieNotFound.left())
        val updatedShow2 = show2.copy(overview = Overview("Updated"))
        whenever(tmdbClient.tvShowDetails(101)).thenReturn(updatedShow2.right())
    ...
    fun `should log error and continue when tmdb fails for a movie`() {
        ...
        whenever(tmdbClient.movieDetails(200)).thenReturn(TmdbError.MovieNotFound.left())
        val updatedMovie2 = movie2.copy(overview = Overview("Updated"))
        whenever(tmdbClient.movieDetails(201)).thenReturn(updatedMovie2.right())
