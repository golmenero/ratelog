...
class SearchTrendingHandlerTest {
    ...
    fun `given tmdb returns movies and tv shows when handle then return interleaved items`() {
        ...
        val query = SearchTrendingQuery(lang = Lang.en)
    ...
    @Test
    fun `given tmdb returns more than 9 per type when handle then limit each list to 9`() {
        val movies = (1..12).map {
        ...
        val trending = result.getOrElse { SearchTrending(emptyList()) }
        assertEquals(18, trending.items.size)
        assertEquals(9, trending.items.count { it.type == MediaType.movie })
        assertEquals(9, trending.items.count { it.type == MediaType.tvshow })
    }
