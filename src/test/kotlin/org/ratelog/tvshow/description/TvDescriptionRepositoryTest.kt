...
class TvDescriptionRepositoryTest {
    ...
    fun `should return description when it exists for tmdbId and lang`() {
        val desc = TvDescription(null,TmdbId(1), Lang.en, Title("Test"), Overview("Overview"))
    ...
    fun `should return all descriptions for a tmdbId`() {
        val descs = listOf(
            TvDescription(null,TmdbId(1), Lang.en, Title("EN Name"), Overview("EN Overview")),
            TvDescription(null,TmdbId(1), Lang.es, Title("ES Name"), Overview("ES Overview")),
            TvDescription(null,TmdbId(2), Lang.en, Title("Other"), null),
        )
    ...
    fun `should return true when at least one description exists for tmdbId`() {
        repository.saveAll(listOf(TvDescription(null,TmdbId(1), Lang.en, Title("Test"), null)))
    ...
    fun `should overwrite existing description when saving with same tmdbId and lang`() {
        val original = TvDescription(null,TmdbId(1), Lang.en, Title("Original"), Overview("Original"))
        repository.saveAll(listOf(original))

        val updated = TvDescription(null,TmdbId(1), Lang.en, Title("Updated"), Overview("Updated"))
